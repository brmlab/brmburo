...
from project.roster.models import Transaction,Currency, Account, Buddy
...
for token in settings.BANK_TOKENS:
    ...
    info = client.info()
    my,created = Account.objects.get_or_create(account_number=info['account_number'], bank_code=info['bank_code'])
    if created:
        my.save()
    logger.info('Processing account %s...' % my)
    try:
        from_id = Transaction.objects.filter(my_account=my).latest('date').tid
    ...
    for tran in trans:
        ...
        acc,created = Account.objects.get_or_create(account_number=tran['account_number'], bank_code=tran['bank_code'])
        if created:
