...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding unique constraint on 'Currency', fields ['symbol']
        db.create_unique(u'roster_currency', ['symbol'])

        # Adding field 'LogicAccount.symbol'
        db.add_column(u'roster_logicaccount', 'symbol',
                      self.gf('django.db.models.fields.CharField')(default='', unique=True, max_length=10),
                      keep_default=False)

        # Adding unique constraint on 'LogicAccountType', fields ['symbol']
        db.create_unique(u'roster_logicaccounttype', ['symbol'])

        # Adding field 'Buddy.logic_account'
        db.add_column(u'roster_buddy', 'logic_account',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['roster.LogicAccount'], null=True, blank=True),
                      keep_default=False)


    def backwards(self, orm):
        # Removing unique constraint on 'LogicAccountType', fields ['symbol']
        db.delete_unique(u'roster_logicaccounttype', ['symbol'])

        # Removing unique constraint on 'Currency', fields ['symbol']
        db.delete_unique(u'roster_currency', ['symbol'])

        # Deleting field 'LogicAccount.symbol'
        db.delete_column(u'roster_logicaccount', 'symbol')

        # Deleting field 'Buddy.logic_account'
        db.delete_column(u'roster_buddy', 'logic_account_id')
